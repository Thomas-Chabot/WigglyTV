...
export function addChannel(channel: TChannel) {
    ...
    addToList(channelsById, source.id, source);
    
    // Add the channel to the map by categories
    for (var category of source.categories) {
        addToList(channels, category, source);
